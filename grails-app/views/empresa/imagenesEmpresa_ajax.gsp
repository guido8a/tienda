...
<div class="alert alert-warning" style="margin-top: 5px;">
    ...
    Se puede cargar hasta <strong>3</strong> imágenes, de tipo <strong>.jpeg, .jpg, .png</strong>
...
<div class=""  style="width: 100%;height: 350px; margin-top: -20px">
    <div id="divImagenes">
    </div>
</div>
...
<script type="text/javascript">
    ...
    function cargarTablaImagenes() {
        $.ajax({
            type: 'POST',
            url: '${createLink(controller: 'empresa', action: 'tablaImagenesEmp_ajax')}',
            data:{
                id: '${empresa?.id}'
            },
        ...
        if(ajaxResponse == 'ok'){
            if (okContents[type]) {
                ...
                if (mb <= 5) {
                    ...
                    request[rs].open("POST", "${g.createLink(controller: 'empresa',action: 'upload_ajax')}");
    ...
    function revisarImas() {
        ...
        return $.ajax({
            type: "POST",
            url: '${createLink(controller: 'empresa', action: 'revisarImas_ajax')}',
            data: {
                id: '${empresa?.id}'
            },
            dataType: "html",
