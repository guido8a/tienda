...
<table class="table table-bordered table-hover table-condensed">
    ...
    <g:each in="${items}" var="item">
        <tr style="width: 100%">
            <td style="width: 20%">${item.prodcdgo}</td>
            <td style="width: 50%">${item.prodtitl}</td>
            ...
            <g:if test="${proceso?.tipoProceso?.codigo?.trim() == 'C'}">
                <td style="width: 5%; text-align: center">
                    <a href="#" class="btn btn-success btnAgregarItem"
                       title="Agregar Item" codigo="${item.prodcdgo}" nombre="${item.prodtitl}" precio="${item.prodpcun}" exis="${item.exst}"  idI="${item.prod__id}" costo="${item.prodpccs}"><i class="fa fa-check"></i></a> </td>
            </g:if>
            <g:else>
